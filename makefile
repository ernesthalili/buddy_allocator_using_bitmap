...
buddy.o: 
	$(CC) $(CCOPTS) $(CC_DUMP) Main.c bit_map.*  Buddy_Allocator.* pool_allocator.* bit_map_tree.* $(LIBS)
...
buddyGProf:
	$(CC) $(CCOPTS) $(CC_GPROF) -o Bud_test Main.c bit_map.* * Buddy_Allocator.* pool_allocator.* bit_map_tree.* $(LIBS)
	./Bud_test
...
BitmapTest:
	$(CC) $(CCOPTS) -o BM_test $(TEST_DIR)Bitmap_test.c Bitmap.* pool_allocator.* bit_map_tree.* $(LIBS)
	./BM_test
	mv BM_test $(BINS_DIR)

BitmapVG:
	$(CC) $(CCOPTS) -o BM_test $(TEST_DIR)Bitmap_test.c bit_map.* pool_allocator.* bit_map_tree.* $(LIBS)
